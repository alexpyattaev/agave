...
mod tests {
    ...
    /// and resumes delivery after the bucket refills, and finally a sustained
    /// flood that drains the whole bucket trips the DoS ban.
    fn test_rate_limit_drops_then_bans_on_sustained_flood() {
        let rt = make_runtime();
        let server = spawn_node(&rt, Arc::new(AllowAll), Keypair::new());
        let client = spawn_node(&rt, Arc::new(AllowAll), Keypair::new());
        ...
        // A SUSTAINED flood that drains the entire bucket, we wait until banned
        let flood = Bytes::from_static(b"flood");
        let banned = rt.block_on(async {
            ...
            while tokio::time::Instant::now() < deadline {
                ...
                if server.banlist.is_banned(&client.pubkey()) {
                    return true;
                }
            }
            server.banlist.is_banned(&client.pubkey())
        ...
        assert!(
            banned,
            "draining the per-connection DoS bucket (PEER_RATE_LIMIT_BURST_DOS) must ban the peer"
        );
    }
}
