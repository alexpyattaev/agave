...
enum StartLeaderError {
    ...
    /// Block completion failed.
    #[error("Block completion failed: {0}")]
    BlockCompletion(#[from] PohRecorderError),
}
...
/// Return the notification with the highest start slot, preserving `current`
/// on ties.
fn latest_window(
    current: Option<LeaderWindowInfo>,
    candidate: Option<LeaderWindowInfo>,
) -> Option<LeaderWindowInfo> {
    if current.as_ref().map(|info| info.start_slot)
        >= candidate.as_ref().map(|info| info.start_slot)
    {
        current
    } else {
        candidate
...
fn freshest_window_from_iter(
    ...
    candidates
        .into_iter()
        .fold(current, |current, candidate| match current {
            Some(current) if current.start_slot > candidate.start_slot => Some(current),
            _ => Some(candidate),
        })
}
...
fn stash_parent_ready(ctx: &mut LeaderContext, info: LeaderWindowInfo) {
    ctx.pending_parent_ready = latest_window(ctx.pending_parent_ready.take(), Some(info));
}

/// Choose between finalized and optimistic leader-window notifications.
///
/// ParentReady wins ties because finalized parent information should override an
/// optimistic parent for the same leader window.
...
fn produce_window(
    ...
    while !ctx.exit.load(Ordering::Relaxed) && slot <= end_slot {
            ...
            record_and_complete_block(ctx, slot, optimistic_parent, &mut block_timer, timeout)
        {
            ...
            return Err(StartLeaderError::BlockCompletion(e));
        }
...
fn record_and_complete_block(
    ...
    if window_has_moved_on {
        let _ = recv_reward_certs(ctx, bank_slot)?;
...
mod tests {
    ...
    fn test_abort_failed_working_bank() {
        ...
        create_and_insert_leader_bank(1, root_bank.clone(), &mut ctx);
        assert!(ctx.poh_recorder.read().unwrap().has_bank());
        ...
        create_and_insert_leader_bank(1, root_bank, &mut ctx);
        assert!(ctx.poh_recorder.read().unwrap().has_bank());
        assert!(ctx.bank_forks.read().unwrap().get(1).is_some());
