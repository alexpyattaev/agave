...
impl ReplayStage {
    ...
    fn restart_slot_from_update_parent(
        ...
        {
            UpdateParentReplayStatus::Ready => {}
            UpdateParentReplayStatus::NotAvailable | UpdateParentReplayStatus::Invalid => {
                return false;
            }
        }
        ...
        if bank.as_ref().is_some_and(|bank| {
            Self::should_skip_own_leader_replay(blockstore, bank, my_pubkey, migration_status)
        }) {
            return false;
        }
        ...
        if let Some(bank) = bank {
            ...
            Self::clear_bank_if_present(bank_forks, signal.slot);
        }
        Self::send_clear_soft_dead_slot(soft_dead_slot_sender, signal.slot);
        true
    }
    ...
    fn process_soft_dead_update_parent_slots(
        ...
        for slot in soft_dead_slots {
            if blockstore.is_dead(slot) {
                ...
                Self::send_clear_soft_dead_slot(soft_dead_slot_sender, slot);
                continue;
            }
